...
public interface GrowingCatm1Repository extends JpaRepository<GrowingCatm1Entity, Integer> {
    ...
    List<GrowingCatm1Entity> findByYear(@Param("year") int year);

    // 월간 통계: 연도와 월 일치
    @Query("SELECT g FROM GrowingCatm1Entity g WHERE FUNCTION('YEAR', g.time) = :year AND FUNCTION('MONTH', g.time) = :month")
    List<GrowingCatm1Entity> findByYearAndMonth(@Param("year") int year, @Param("month") int month);

    // 주간 통계: 특정 날짜 범위 (startDate ~ endDate)
    @Query("SELECT g FROM GrowingCatm1Entity g WHERE g.time BETWEEN :startDate AND :endDate")
    List<GrowingCatm1Entity> findByDateRange(@Param("startDate") LocalDateTime startDate, @Param("endDate") LocalDateTime endDate);

    // 일간 통계: 특정 연도-월-일 일치
    @Query("SELECT g FROM GrowingCatm1Entity g WHERE FUNCTION('YEAR', g.time) = " +
            ":year AND FUNCTION('MONTH', g.time) = :month AND FUNCTION('DAY', g.time) = :day")
    List<GrowingCatm1Entity> findByYearMonthAndDay(@Param("year") int year, @Param("month") int month, @Param("day") int day);
}
