...
public interface MaternityCatm1Repository extends JpaRepository<MaternityCatm1Entity, Integer>{
    // 연간 통계: 연도만 일치
    @Query("SELECT g FROM MaternityCatm1Entity g WHERE g.time LIKE CONCAT(:year, '%')")
    List<MaternityCatm1Entity> findByYear(@Param("year") String year);

    // 월간 통계: 연도와 월 일치
    @Query("SELECT g FROM MaternityCatm1Entity g WHERE g.time LIKE CONCAT(:year, '-', :month, '%')")
    ...
    // 일간 통계: 특정 연도-월-일 일치
    @Query("SELECT g FROM MaternityCatm1Entity g WHERE g.time LIKE CONCAT(:year, '-', :month, '-', :day, '%')")
