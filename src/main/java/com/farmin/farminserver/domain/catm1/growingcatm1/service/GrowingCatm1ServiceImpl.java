    ...
    public List<GrowingCatm1Response> getStatistics(String type, String year, String month, String weekOrDay) {
        ...
        switch (type.toLowerCase()) {
            case "yearly" -> {
                ...
                catm1 = growingcatm1Repository.findByYear(parsedYear);
            }
            case "monthly" -> {
                int parsedYear = Integer.parseInt(year);
                int parsedMonth = Integer.parseInt(month);
                catm1 = growingcatm1Repository.findByYearAndMonth(parsedYear, parsedMonth);
            }
            case "weekly" -> {
                ...
                catm1 = growingcatm1Repository.findByDateRange(start, end);
            }
            case "daily" -> {
                int parsedYear = Integer.parseInt(year);
                int parsedMonth = Integer.parseInt(month);
                int parsedDay = Integer.parseInt(weekOrDay);
                catm1 = growingcatm1Repository.findByYearMonthAndDay(parsedYear, parsedMonth, parsedDay);
            }
